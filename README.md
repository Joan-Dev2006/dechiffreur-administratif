...
# Mettons en place une écriture propre ligne par ligne pour éviter les conflits de triple-guillemets.

lines = [
    "# 📦 KIT DE DÉPLOIEMENT COMPLET : LE DÉCHIFFREUR ADMINISTRATIF",
    "",
    "Ce document rassemble l'intégralité des fichiers nécessaires pour faire tourner votre application. Vous pouvez copier-coller chaque section directement dans les fichiers correspondants sur votre ordinateur.",
    "",
    "---",
    "",
...
```text",
    "Dechiffreur_Paperasse/",
    "├── .streamlit/",
    "│   └── secrets.toml        # Votre clé API secrète",
    "├── app.py                  # Le code principal de l'application Streamlit",
    "└── README.md               # La documentation pour GitHub",
    "```",
    "",
    "---",
    "",
    "## 🛠️ FICHIER 1 : `app.py`",
    "**Instructions :** Créez un fichier nommé `app.py` à la racine de votre dossier et collez-y tout le code ci-dessous.",
    "",
    "```python",
    "import sys",
    "import asyncio",
    "import logging",
    "",
    "# 🤫 SILENCIEUX RÉSEAU : Cache les erreurs fantômes de déconnexion (WinError 10054) propres à Windows",
    ...
    "",
    "# --- DESIGN CSS PERSONNALISÉ (STYLE PREMIUM UI) ---",
    "st.markdown(\"\"\"",
    "<style>",
    "    /* Arrière-plan global et typographie */",
    ...
    "    ",
    "    /* Bouton principal avec un dégradé moderne et effet de survol */",
    "    .stButton>button {",
    ...
    "    }",
    "    ",
    "    /* Design épuré des onglets de sélection */",
    ...
    "    st.stop()",
    "",
    "# Barre latérale (Sidebar) au look institutionnel",
    "with st.sidebar:",
    ...
    "    st.caption(\"- Vision : Moteur OCR Local Multi-langue (CPU)\\n- Cerveau : Llama 3.3 70B Versatile\\n- Température : 0.0 (Précision factuelle stricte)\")",
    "",
    "# --- FONCTIONS DE TRAITEMENT ---",
    "def anonymiser_texte(texte_brut):",
    "    \"\"\"Masque les e-mails et téléphones français avant l'envoi au cloud\"\"\"",
    ...
    "    \"\"\"Extrait le texte des fichiers texte, PDF ou Word\"\"\"",
    ...
    "    \"\"\"Utilise EasyOCR en local pour lire le texte sur les images (FR, EN, ES)\"\"\"",
    ...
    "    \"✍️ Copier-coller du texte brut\"",
    "])",
    "",
    "texte_a_analyser = \"\"",
    "",
    "with onglet_photo:",
    "    photo_capturee = st.camera_input(\"Alignez bien le document face à l'objectif et déclenchez :\")",
    "    if photo_capturee is not None:",
    "        with st.spinner(\"👁️ Numérisation et lecture locale de l'image en cours...\"):",
    "            texte_a_analyser = extraire_texte_via_vision_locale(photo_capturee)",
    "",
    "with onglet_fichier:",
    "    fichier_implique = st.file_uploader(\"Glissez votre document ici (Formats : PDF, DOCX, TXT, PNG, JPG) :\", type=[\"pdf\", \"docx\", \"txt\", \"png\", \"jpg\", \"jpeg\"])",
    "    if fichier_implique is not None:",
    "        extension = fichier_implique.name.split(\".\")[-1].lower()",
    "        if extension in [\"png\", \"jpg\", \"jpeg\"]:",
    "            with st.spinner(\"👁️ Analyse locale de l'image importée...\"):",
    ...
    "",
    "with onglet_texte:",
    "    texte_colle = st.text_area(\"Collez le bloc de texte ici :\", height=150, placeholder=\"Ex: Lettre de relance, notification de l'administration...\")",
    "    if texte_colle.strip():",
    "        texte_a_analyser = texte_colle",
    "",
    "# --- BLOC DE DIAGNOSTIC ET MOTEUR D'ANALYSE ---",
    "if texte_a_analyser.strip():",
    "    texte_securise = anonymiser_texte(texte_a_analyser)",
    "    ",
    "    st.markdown(\"### 🛡️ 2. Validation & Sécurité\")",
    "    st.success(\"✓ Document numérisé et sécurisé en mémoire locale.\")",
    "    ",
    "    with st.expander(\"🔍 Inspecter le texte brut extrait (Vérification OCR)\"):",
    ...
    "                    pass",
    "else:",
    "    st.info(\"💡 **Guide de démarrage rapide :** Choisissez un onglet ci-dessus pour soumettre une photo, un fichier ou un texte. L'IA se chargera de le traduire instantanément en instructions claires.\")",
    "```",
    "",
    "---",
    "",
    "## 🔑 FICHIER 2 : `.streamlit/secrets.toml`",
    "**Instructions :** Créez un dossier nommé `.streamlit` à la racine de votre projet, créez un fichier nommé `secrets.toml` à l'intérieur, et collez-y votre clé.",
    "",
    "```toml",
    "GROQ_API_KEY = \"VOTRE_VERITABLE_CLE_API_GROQ\"",
    "```",
    "",
    "---",
    "",
    "## 📝 FICHIER 3 : `README.md`",
    "**Instructions :** Créez un fichier nommé `README.md` à la racine de votre dossier pour documenter proprement votre projet sur GitHub.",
    "",
    "```markdown",
    "# ⚖️ Le Déchiffreur Administratif",
    "",
    "Une application Web intuitive conçue pour traduire le jargon administratif complexe en une feuille de route claire, actionnable et chronologique. ",
    "",
    "L'application adopte une **architecture hybride** : elle extrait le texte des images/photos localement via `EasyOCR` pour garantir la gratuité et la flexibilité, puis s'appuie sur la puissance de l'API Groq (`Llama 3.3 70B`) pour l'analyse sémantique.",
    "",
    "---",
    "",
    "## ✨ Fonctionnalités clés",
    "* **📸 Scanner Photo Grand Public :** Capture en direct via smartphone ou webcam (`EasyOCR` local).",
    "* **📁 Multi-formats :** Support des PDF, Word (`.docx`), fichiers texte et images (`PNG`, `JPG`).",
    "* **🧠 Analyse Haute Précision :** Tri automatique en 3 piliers (Sens, Actions, Échéances) via `llama-3.3-70b-versatile`.",
    "* **🛡️ Sécurisé & Conforme RGPD :** Anonymisation automatique locale des données sensibles (téléphones, e-mails) par Regex avant transfert au LLM.",
    "",
    "---",
    "",
    ...
    "```",
    "",
    "### 2. Installer les dépendances d'IA",
    "```bash",
...
with open("kit_deploiement_dechiffreur.md", "w", encoding="utf-8") as f:
    f.write(full_text)
